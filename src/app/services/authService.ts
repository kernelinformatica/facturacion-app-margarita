...
import { FiltroComprobantePago, FiltroOrdenPago, GrabarOrdenesPago } from 'app/models/comprobantes-pago';
...
export class AuthService {
    ...
    * @argument filtros Lo filtro
    */
    getProductosPendientes = (token: string) => (proveedor: Padron) => (comproRel: ComprobanteRelacionado) => (comprobante: Comprobante) => (tipoOpSelect: SisTipoOperacion) => (listaPrecioSelect: ListaPrecio) => (modulo) => (diferenciaFechas?) => (codigoCereal?) => (esCanje?) => (esPesificado?) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
    ...
    */
    ...
    buscaPendientesProd = (token, idCteTipo, facNumero, codigoProv, idMoneda, idSisOperacionComprobante, letra, idSisTipoOperacion, codProd) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
    ...
    buscaComprobanteAfip = (token, codigoAfipRelacionado, numeroAfipRelacionado) => {
        return this.request(
            [],
            RequestMethod.Get,
            {
            ...
            resourcesREST.buscaComprobantes.nombre + "?&codigoAfipRelacionado=" + codigoAfipRelacionado + "&numeroAfipRelacionado=" + numeroAfipRelacionado,
            {
     ...
     */
    buscaModelos = (token, productos: ProductoBuscaModelo[], idSisModulo, idMoneda, idPadron, idSisTipoOperacion?, tipoComprobante?, estadoSisa?, esCanje?) =>
        this.request(
            [],
            RequestMethod.Post,
            {
    ...
    abmCerealesCanje = (token, codigoFuncion, ctaContable, cerealCodigo, idCanjeContratoCereal, idEmpresa) =>
        this.request(
            [],
            RequestMethod.Post,
            {
    ...
    abmParametrosCanje = (token, codigoFuncion, interesDiario, diasLibres, cerealCodigo, ctaContableSisa, idEmpresa, idParametrosCanje) =>
        this.request(
            [],
            RequestMethod.Post,
            {
                                                                ...
                                                                (tipoCambio: string) => {
                                                                    return this.request(
                                                                        ...
                                                                        'grabaComprobante',
                                                                        {
                                                                            ...
                                                                            fechaVencimiento: this.utilsService.formatearFecha('yyyy-mm-dd')(comprobante.fechaVto),
                                                                            ...
                                                                            fechaVencimientoFact: factura ? this.utilsService.formatearFecha('yyyy-mm-dd')(comprobante.fechaVto) : null,
                                                                            ...
                                                                            interesMensualCompra: interesMensualCompra ? interesMensualCompra : 0,
                                                                                                ...
                                                                                                (nroCompPesificado?) => {
                                                                                                    return this.request(
                                                                                                        ...
                                                                                                        'grabaComprobante',
                                                                                                        {
                                                                                                            ...
                                                                                                            pesificadoPersisteSn: pesificadoPersisteSn ? pesificadoPersisteSn : false,
                                                                                                            ...
                                                                                                            grillaArticulos: productosPend.map(prod => {
                                                                                                                ...
                                                                                                                const subtotalBuscado = subtotalesProductos
                                                                                                                    .find(
                                                                                                                        st => {
                                                                                                                            return st.idProducto === prod.producto.idProductos &&
    ...
    descargarComprobante = (token) => (idFactCab) => (nombrePdf?) => (nroCopias?) => (esCanje?) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
    ...
    */
    ...
    reportePosStock = (token: string) => (fechaDesde) => (fechaHasta) => (codProducto) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
            ...
            resourcesREST.buscaPosicionStock.nombre + "/particular/imprimir",
            {
                fechaDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(fechaDesde),
                fechaHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(fechaHasta),
                codProducto
            },
            {},
            true
    ...
    */
    ...
    buscaComprobantesPesificacion = (token: string) =>
        (fechasFiltro) =>
            (padronSelec: Padron) => {
                return this.request(
                    [],
                    ...
                    {
                        fechaEmisionDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.emisionDesde),
                        fechaEmisionHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.emisionHasta),
                        padCodigo: padronSelec && padronSelec.padronCodigo ? padronSelec.padronCodigo : 0,
                        fechaVencimientoDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.vencimientoDesde),
                        fechaVencimientoHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.vencimientoHasta),
                    },
                    {}
                );
            }

    buscaComprobantePesificacion = (token: string) =>
        (numeroComprobante) => {
            return this.request(
                [],
                RequestMethod.Post,
    ...
    * @argument filtros Lo filtro
    */
    buscaComprobantesAnticipado = (token: string) => (comprobante: Comprobante) =>
        (fechasFiltro: { desde: DateLikePicker, hasta: DateLikePicker }) =>
            (sisModuloSelec: SisModulo) =>
                (tipoComprobanteSelec: TipoComprobante) =>
                                                        ...
                                                        (productoHasta?: string) => {
                                                            return this.request(
                                                                ...
                                                                resourcesREST.buscaComprobantesAnticipados.nombre,
                                                                {
                                                                    comprobanteModulo: sisModuloSelec && sisModuloSelec.idSisModulos ? sisModuloSelec.idSisModulos : 0,
                                                                    comprobanteTipo: tipoComprobanteSelec && tipoComprobanteSelec.idCteTipo ? tipoComprobanteSelec.idCteTipo : 0,
                                                                    comprobanteTipo2: tipoComprobanteSelec2 && tipoComprobanteSelec2.idCteTipo ? tipoComprobanteSelec2.idCteTipo : 0,
                                                                    comprobanteNumero: comprobante && comprobante.numerador.ptoVenta.ptoVenta && comprobante.numerador.numerador ? `${comprobante.numerador.ptoVenta.ptoVenta}${comprobante.numerador.numerador}` : 0,
                                                                    fechaDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.desde),
                                                                    fechaHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(fechasFiltro.hasta),
                                                                    idProducto: productoSelec && productoSelec.idProductos ? productoSelec.idProductos : 0,
    ...
    getPosicionStockGral = (token: string) => (fechasFiltro: { desde: DateLikePicker, hasta: DateLikePicker }) =>
        (productoDesde?: string) =>
            (productoHasta?: string) => {
                return this.request(
                    [],
    ...
    getPosicionStock = (token: string) => (fechasFiltro: { desde: DateLikePicker, hasta: DateLikePicker }) =>
        (producto: string) => {
            return this.request(
                [],
                RequestMethod.Post,
    ...
    buscaComprobantesPadronAnticipada = (token: string) =>
        (padron) => {
            ...
            return this.request(
                ...
                resourcesREST.buscaComprobantes.nombre,
                {
                    ...
                    comprobanteTipo: 75,
                    comprobanteNumero: 0,
                    fechaDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(new DateLikePicker(fechaAntes)),
                    fechaHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(new DateLikePicker(new Date())),
                    idProducto: 0,
                    padCodigo: padron,
                    idDeposito: 0,
                    idEstado: 1,
    ...
    actualizarProductosViejoANuevo = (token: string, codigoProductoDesde, codigoProductoHasta) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
    ...
    */
    getBuscaStock = (token: string) => (filtros: {
        ...
        return this.request(
            ...
            {
                fechaDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(filtros.fechaDesde),
    ...
    * @argument idProducto
    */
    getBuscarProducto = (token: string) => (idProducto: any) => (idListaPrecio?) => (idMoneda) => (idSisTipoOperacion?) => (idCteTipo?) => (diferenciaFechas?) => (esCanje?) => (codigoCereal?) => (esPesificado?) => {
        return this.request(
            [idProducto],
            RequestMethod.Get,
            {
    ...
    setImputaciones = (token, idFactDetalle, idFactDetalleImputa, cantidadImputada) => {
        return this.request(
            [],
            RequestMethod.Post,
            {
    ...
    buscarOrdenesPago = (token, filters: FiltroOrdenPago) => {
        return this.request(
            ...
            'buscarOrdenesPago',
            {
                ...
                fechaDesde: this.utilsService.formatearFecha('yyyy-mm-dd')(filters.fechaDesde),
                fechaHasta: this.utilsService.formatearFecha('yyyy-mm-dd')(filters.fechaHasta),
            },
            {}
        ).toPromise();
    }
